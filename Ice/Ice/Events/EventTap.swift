...
final class EventTap {
    ...
    init(
        ...
        guard let machPort = Self.createTapMachPort(
            ...
            Bridging.Logger.eventTap.error("Error creating mach port for event tap \"\(self.label)\"")
            return
        }
        guard let source = CFMachPortCreateRunLoopSource(nil, machPort, 0) else {
            Bridging.Logger.eventTap.error("Error creating run loop source for event tap \"\(self.label)\"")
            return
        }
    ...
    private func withUnwrappedComponents(body: @MainActor (CFRunLoop, CFRunLoopSource, CFMachPort) -> Void) {
        guard let runLoop else {
            Bridging.Logger.eventTap.error("Missing run loop for event tap \"\(self.label)\"")
            return
        }
        guard let source else {
            Bridging.Logger.eventTap.error("Missing run loop source for event tap \"\(self.label)\"")
            return
        }
        guard let machPort else {
            Bridging.Logger.eventTap.error("Missing mach port for event tap \"\(self.label)\"")
            return
        }
...
private extension Bridging.Logger {
    static let eventTap = Bridging.Logger(category: "EventTap")
}
