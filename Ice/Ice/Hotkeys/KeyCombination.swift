...
private func getSystemReservedKeyCombinations() -> [KeyCombination] {
    ...
    guard status == noErr else {
        Bridging.Logger.keyCombination.error("CopySymbolicHotKeys returned invalid status: \(status)")
        return []
    }
    guard let reservedHotkeys = symbolicHotkeys?.takeRetainedValue() as? [[String: Any]] else {
        Bridging.Logger.keyCombination.error("Failed to serialize symbolic hotkeys")
        return []
    }
...
private extension Bridging.Logger {
    static let keyCombination = Bridging.Logger(category: "KeyCombination")
}
