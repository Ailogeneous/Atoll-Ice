...
final class MenuBarAppearanceManager: ObservableObject {
    ...
    private func loadInitialState() {
        do {
            if let data = IceDefaults.data(forKey: .menuBarAppearanceConfigurationV2) {
                configuration = try decoder.decode(MenuBarAppearanceConfigurationV2.self, from: data)
            }
        } catch {
            Bridging.Logger.appearanceManager.error("Error decoding configuration: \(error)")
        }
    }
        ...
        $configuration
            ...
            .sink { completion in
                if case .failure(let error) = completion {
                    Bridging.Logger.appearanceManager.error("Error encoding configuration: \(error)")
                }
            } receiveValue: { data in
                IceDefaults.set(data, forKey: .menuBarAppearanceConfigurationV2)
            }
            .store(in: &c)
...
private extension Bridging.Logger {
    ...
    static let appearanceManager = Bridging.Logger(category: "MenuBarAppearanceManager")
}
