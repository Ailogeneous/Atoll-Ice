...
final class AppState: ObservableObject {
    ...
    /// The app's delegate.
    private(set) weak var appDelegate: (any NSApplicationDelegate)?
    ...
    private func configureCancellables() {
        ...
        if let settingsWindow {
            ...
            Bridging.Logger.appState.warning("No settings window!")
        }
    ...
    func assignAppDelegate(_ appDelegate: any NSApplicationDelegate) {
        guard self.appDelegate == nil else {
            Bridging.Logger.appState.warning("Multiple attempts made to assign app delegate")
            return
        }
    ...
    func assignSettingsWindow(_ window: NSWindow) {
        guard window.identifier?.rawValue == IceConstants.settingsWindowID else {
            Bridging.Logger.appState.warning("Window \(window.identifier?.rawValue ?? "<NIL>") is not the settings window!")
            return
        }
    ...
    func assignPermissionsWindow(_ window: NSWindow) {
        guard window.identifier?.rawValue == IceConstants.permissionsWindowID else {
            Bridging.Logger.appState.warning("Window \(window.identifier?.rawValue ?? "<NIL>") is not the permissions window!")
            return
        }
    ...
    func dismissSettingsWindow() {
        with(EnvironmentValues()) { environment in
            environment.dismissWindow(id: IceConstants.settingsWindowID)
        }
    }
    ...
    func openPermissionsWindow() {
        with(EnvironmentValues()) { environment in
            environment.openWindow(id: IceConstants.permissionsWindowID)
        }
    }
    ...
    func dismissPermissionsWindow() {
        with(EnvironmentValues()) { environment in
            environment.dismissWindow(id: IceConstants.permissionsWindowID)
        }
    }
    ...
    func activate(withPolicy policy: NSApplication.ActivationPolicy) {
        ...
        if Context.hasActivated.value(for: self) == true {
            ...
            Bridging.Logger.appState.debug("First time activating app, so going through Dock")
...
private extension Bridging.Logger {
    ...
    static let appState = Bridging.Logger(category: "AppState")
}
