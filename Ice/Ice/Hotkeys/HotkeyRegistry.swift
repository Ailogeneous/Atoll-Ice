...
final class HotkeyRegistry {
    ...
    func register(hotkey: Hotkey, eventKind: EventKind, handler: @escaping () -> Void) -> UInt32? {
        ...
        guard let keyCombination = hotkey.keyCombination else {
            Bridging.Logger.hotkeyRegistry.error("Hotkey does not have a valid key combination")
            return nil
        }
        ...
        guard status == noErr else {
            Bridging.Logger.hotkeyRegistry.error("Hotkey event handler installation failed with status \(status)")
            return nil
        }
        ...
        guard registrations[id] == nil else {
            Bridging.Logger.hotkeyRegistry.error("Hotkey already registered for id \(id)")
            return nil
        }
        ...
        guard status == noErr else {
            Bridging.Logger.hotkeyRegistry.error("Hotkey registration failed with status \(status)")
            return nil
        }

        guard let hotKeyRef else {
            Bridging.Logger.hotkeyRegistry.error("Hotkey registration failed due to invalid EventHotKeyRef")
            return nil
        }
    ...
    private func retainedUnregister(_ id: UInt32) {
        guard let registration = registrations[id] else {
            Bridging.Logger.hotkeyRegistry.error("No registered key combination for id \(id)")
            return
        }
        let status = UnregisterEventHotKey(registration.hotKeyRef)
        guard status == noErr else {
            Bridging.Logger.hotkeyRegistry.error("Hotkey unregistration failed with status \(status)")
            return
        }
...
private extension Bridging.Logger {
    static let hotkeyRegistry = Bridging.Logger(category: "HotkeyRegistry")
}
