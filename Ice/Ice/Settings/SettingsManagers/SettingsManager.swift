...
final class SettingsManager: ObservableObject {
    ...
    func performSetup() {
        ...
        recoverAccessIfUserDisabledAllEntryPoints()
    }
...
private extension SettingsManager {
    func recoverAccessIfUserDisabledAllEntryPoints() {
        guard !generalSettingsManager.showIceIcon,
              !advancedSettingsManager.showContextMenuOnRightClick else {
            return
        }

        advancedSettingsManager.showContextMenuOnRightClick = true
        Bridging.Logger.settingsManager.warning("Recovered Ice access by enabling right-click context menu because all entry points were disabled")
    }
}

private extension Bridging.Logger {
    static let settingsManager = Bridging.Logger(category: "SettingsManager")
}
