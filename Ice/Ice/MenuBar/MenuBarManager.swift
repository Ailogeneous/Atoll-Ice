...
final class MenuBarManager: ObservableObject {
    ...
    private func initializeSections() {
        ...
        guard sections.isEmpty else {
            Bridging.Logger.menuBarManager.warning("Sections already initialized")
            return
        }

        guard let appState else {
            Bridging.Logger.menuBarManager.error("Error initializing menu bar sections: Missing app state")
            return
        }
            ...
            let window = hiddenSection.controlItem.window
        {
            window.publisher(for: \.frame)
                ...
                .sink { [weak self] _ in
                    guard
                        let self,
                        let isMenuBarHidden = IceDefaults.globalDomain["_HIHideMenuBar"] as? Bool
                    else {
                        return
                    }
    ...
    func hideApplicationMenus() {
        guard let appState else {
            Bridging.Logger.menuBarManager.error("Error hiding application menus: Missing app state")
            return
        }
        Bridging.Logger.menuBarManager.info("Hiding application menus")
    ...
    func showApplicationMenus() {
        guard let appState else {
            Bridging.Logger.menuBarManager.error("Error showing application menus: Missing app state")
            return
        }
        Bridging.Logger.menuBarManager.info("Showing application menus")
    ...
    @objc private func showAppearanceEditorPopover() {
        guard let appState else {
            Bridging.Logger.menuBarManager.error("Error showing appearance editor popover: Missing app state")
            return
        }
...
private extension Bridging.Logger {
    /// Bridging.Logger to use for the menu bar manager.
    static let menuBarManager = Bridging.Logger(category: "MenuBarManager")
}
