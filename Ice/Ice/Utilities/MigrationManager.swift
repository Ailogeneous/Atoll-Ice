...
extension MigrationManager {
    ...
    private static func logError(_ error: any Error) {
        Bridging.Logger.migration.error("Migration failed with error: \(error)")
    }
}
...
extension MigrationManager {
    ...
    private func migrate0_8_0() throws {
        guard !IceDefaults.bool(forKey: .hasMigrated0_8_0) else {
            return
        }
        ...
        IceDefaults.set(true, forKey: .hasMigrated0_8_0)
        Bridging.Logger.migration.info("Successfully migrated to 0.8.0 settings")
    }
    ...
    private func migrateControlItems0_8_0() throws {
        ...
        do {
            ...
            IceDefaults.set(data, forKey: .sections)
        } catch {
            throw MigrationError.controlItemMigrationError(error)
        }
    }
    ...
    private func migrateSections0_8_0() {
        IceDefaults.set(nil as Any?, forKey: .sections)
    }
}
...
extension MigrationManager {
    ...
    private func migrate0_10_0() throws {
        guard !IceDefaults.bool(forKey: .hasMigrated0_10_0) else {
            return
        }
        try MigrationManager.performAll(blocks: [
            migrateControlItems0_10_0,
        ])
        IceDefaults.set(true, forKey: .hasMigrated0_10_0)
        Bridging.Logger.migration.info("Successfully migrated to 0.10.0 settings")
    }
...
extension MigrationManager {
    ...
    private func migrate0_10_1() -> MigrationResult {
        guard !IceDefaults.bool(forKey: .hasMigrated0_10_1) else {
            return .success
        }
        let result = migrateControlItems0_10_1()
        switch result {
        case .success, .successButShowAlert:
            IceDefaults.set(true, forKey: .hasMigrated0_10_1)
            Bridging.Logger.migration.info("Successfully migrated to 0.10.1 settings")
...
extension MigrationManager {
    private func migrate0_11_10() -> MigrationResult {
        guard !IceDefaults.bool(forKey: .hasMigrated0_11_10) else {
            return .success
        }
        let result = migrateAppearanceConfiguration0_11_10()
        switch result {
        case .success, .successButShowAlert:
            IceDefaults.set(true, forKey: .hasMigrated0_11_10)
            Bridging.Logger.migration.info("Successfully migrated to 0.11.10 settings")
        case .failureAndLogError:
            break
        }
        return result
    ...
    private func migrateAppearanceConfiguration0_11_10() -> MigrationResult {
        guard let oldData = IceDefaults.data(forKey: .menuBarAppearanceConfiguration) else {
        ...
        do {
            ...
            IceDefaults.set(newData, forKey: .menuBarAppearanceConfigurationV2)
        } catch {
...
extension MigrationManager {
    ...
    private func getMenuBarSectionArray() throws -> [[String: Any]]? {
        guard let data = IceDefaults.data(forKey: .sections) else {
            return nil
        }
...
private extension Bridging.Logger {
    static let migration = Bridging.Logger(category: "Migration")
}
