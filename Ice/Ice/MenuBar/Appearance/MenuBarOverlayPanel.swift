...
final class MenuBarOverlayPanel: NSPanel {
    ...
    private func validate(for kind: ValidationKind, with windows: [WindowInfo]) -> CGDirectDisplayID? {
        ...
        guard let appState else {
            Bridging.Logger.overlayPanel.debug("No app state. \(actionMessage)")
            return nil
        }
        guard !appState.menuBarManager.isMenuBarHiddenBySystemUserDefaults else {
            Bridging.Logger.overlayPanel.debug("Menu bar is hidden by system. \(actionMessage)")
            return nil
        }
        guard !appState.isActiveSpaceFullscreen else {
            Bridging.Logger.overlayPanel.debug("Active space is fullscreen. \(actionMessage)")
            return nil
        }
        let owningDisplay = owningScreen.displayID
        guard appState.menuBarManager.hasValidMenuBar(in: windows, for: owningDisplay) else {
            Bridging.Logger.overlayPanel.debug("No valid menu bar found. \(actionMessage)")
            return nil
        }
    ...
    private func show() {
        ...
        guard appState.appearanceManager.overlayPanels.contains(self) else {
            Bridging.Logger.overlayPanel.warning("Overlay panel \(self) not retained")
            return
        }
...
private extension Bridging.Logger {
    static let overlayPanel = Bridging.Logger(category: "MenuBarOverlayPanel")
}
