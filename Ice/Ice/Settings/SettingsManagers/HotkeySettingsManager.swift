...
final class HotkeySettingsManager: ObservableObject {
    ...
    private func loadInitialState() {
        if let dict = IceDefaults.dictionary(forKey: .hotkeys) as? [String: Data] {
            for hotkey in hotkeys {
                if let data = dict[hotkey.action.rawValue] {
                    do {
                        hotkey.keyCombination = try decoder.decode(KeyCombination?.self, from: data)
                    } catch {
                        Bridging.Logger.hotkeySettingsManager.error("Error decoding hotkey: \(error)")
                    }
                }
            }
        }
            ...
            .sink { [weak self] hotkeys, _ in
                ...
                for hotkey in hotkeys {
                    ...
                    do {
                        ...
                        Bridging.Logger.hotkeySettingsManager.error("Error encoding hotkey: \(error)")
                    }
                }
                IceDefaults.set(dict, forKey: .hotkeys)
            }
            .store(in: &c)
...
private extension Bridging.Logger {
    static let hotkeySettingsManager = Bridging.Logger(category: "HotkeySettingsManager")
}
