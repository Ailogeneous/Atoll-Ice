...
struct MenuBarAppearanceConfigurationV1: Hashable {
    ...
    static func migrate(encoder: JSONEncoder, decoder: JSONDecoder) throws -> Self {
        ...
        if let data = IceDefaults.data(forKey: .menuBarAppearanceConfiguration) {
            return try decoder.decode(Self.self, from: data)
        } else {
            var configuration = Self.defaultConfiguration

            IceDefaults.ifPresent(key: .menuBarHasShadow, assign: &configuration.hasShadow)
            IceDefaults.ifPresent(key: .menuBarHasBorder, assign: &configuration.hasBorder)
            IceDefaults.ifPresent(key: .menuBarBorderWidth, assign: &configuration.borderWidth)
            IceDefaults.ifPresent(key: .menuBarTintKind) { rawValue in
                if let tintKind = MenuBarTintKind(rawValue: rawValue) {
                    configuration.tintKind = tintKind
                }
            }

            if let borderColorData = IceDefaults.data(forKey: .menuBarBorderColor) {
                configuration.borderColor = try decoder.decode(CodableColor.self, from: borderColorData).cgColor
            }
            if let tintColorData = IceDefaults.data(forKey: .menuBarTintColor) {
                configuration.tintColor = try decoder.decode(CodableColor.self, from: tintColorData).cgColor
            }
            if let tintGradientData = IceDefaults.data(forKey: .menuBarTintGradient) {
                configuration.tintGradient = try decoder.decode(CustomGradient.self, from: tintGradientData)
            }
            if let shapeKindData = IceDefaults.data(forKey: .menuBarShapeKind) {
                configuration.shapeKind = try decoder.decode(MenuBarShapeKind.self, from: shapeKindData)
            }
            if let fullShapeData = IceDefaults.data(forKey: .menuBarFullShapeInfo) {
                configuration.fullShapeInfo = try decoder.decode(MenuBarFullShapeInfo.self, from: fullShapeData)
            }
            if let splitShapeData = IceDefaults.data(forKey: .menuBarSplitShapeInfo) {
            ...
            IceDefaults.set(configurationData, forKey: .menuBarAppearanceConfiguration)

            // Remove the deprecated keys.
            let keys: [IceDefaultsKey] = [
                .menuBarHasShadow,
                .menuBarHasBorder,
                .menuBarBorderWidth,
                .menuBarTintKind,
            ...
            for key in keys {
                IceDefaults.removeObject(forKey: key)
            }
