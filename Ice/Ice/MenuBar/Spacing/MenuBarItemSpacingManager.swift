...
final class MenuBarItemSpacingManager {
    ...
    private func signalAppToQuit(_ app: NSRunningApplication) async throws {
        if app.isTerminated {
            Bridging.Logger.spacing.debug("Application \"\(logString(for: app))\" is already terminated")
            return
        } else {
            Bridging.Logger.spacing.debug("Signaling application \"\(logString(for: app))\" to quit")
        }
        ...
        return try await withCheckedThrowingContinuation { continuation in
            let timeoutTask = Task {
                ...
                if !app.isTerminated {
                    Bridging.Logger.spacing.debug("Application \"\(logString(for: app))\" did not terminate within \(forceTerminateDelay) seconds, attempting to force terminate")
                    app.forceTerminate()
                }
            }
            ...
            cancellable = app.publisher(for: \.isTerminated).sink { [weak self] isTerminated in
                ...
                timeoutTask.cancel()
                cancellable?.cancel()
                Bridging.Logger.spacing.debug("Application \"\(logString(for: app))\" terminated successfully")
                continuation.resume()
            }
        }
    }
    ...
    private nonisolated func launchApp(at applicationURL: URL, bundleIdentifier: String) async throws {
        if let app = NSWorkspace.shared.runningApplications.first(where: { $0.bundleIdentifier == bundleIdentifier }) {
            Bridging.Logger.spacing.debug("Application \"\(logString(for: app))\" is already open, so skipping launch")
            return
        }
...
private extension Bridging.Logger {
    static let spacing = Bridging.Logger(category: "Spacing")
}
