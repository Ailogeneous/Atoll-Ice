...
final class MenuBarItemImageCache: ObservableObject {
    ...
    private func logSkippingCache(reason: String) {
        Bridging.Logger.imageCache.debug("Skipping menu bar item image cache as \(reason)")
    }
    ...
    func createImages(for section: MenuBarSection.Name, screen: NSScreen) async -> [MenuBarItemInfo: CGImage] {
            ...
            CGFloat(compositeImage.width) == frame.width * backingScaleFactor
        {
            ...
            Bridging.Logger.imageCache.warning("Composite image capture failed. Attempting to capturing items individually.")
    ...
    func updateCacheWithoutChecks(sections: [MenuBarSection.Name]) async {
        ...
        for section in sections {
            ...
            guard !sectionImages.isEmpty else {
                Bridging.Logger.imageCache.warning("Update image cache failed for \(section.logString)")
                continue
            }
...
private extension Bridging.Logger {
    static let imageCache = Bridging.Logger(category: "MenuBarItemImageCache")
}
