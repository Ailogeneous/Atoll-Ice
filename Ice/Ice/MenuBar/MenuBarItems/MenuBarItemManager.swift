...
final class MenuBarItemManager: ObservableObject {
    ...
    private struct TempShownItemContext {
        ...
        /// The window identifier associated with the item when it was temporarily shown.
        let windowID: CGWindowID
        /// The owner process identifier associated with the item when it was temporarily shown.
        let ownerPID: pid_t
        /// The item title associated with the item when it was temporarily shown.
        let title: String?
        ...
        /// A Boolean value indicating whether rehide should stop waiting for interface closure.
        var shouldForceRehide: Bool {
...
extension MenuBarItemManager {
    /// Forces a menu bar item cache refresh by bypassing the unchanged-window guard.
    func forceRefreshCache(clearExisting: Bool = false) async {
        Bridging.Logger.itemManager.debug("Forcing menu bar item cache refresh")
        cachedItemWindowIDs.removeAll()
        if clearExisting {
            Bridging.Logger.itemManager.debug("Clearing menu bar item cache before forced refresh")
            itemCache.clear()
        }
    ...
    private func logNotCachedWarning(for item: MenuBarItem) {
        Bridging.Logger.itemManager.warning("\(item.logString) was not cached")
    }

    /// Logs a reason for skipping the cache.
    private func logSkippingCache(reason: String) {
        Bridging.Logger.itemManager.debug("Skipping menu bar item cache as \(reason)")
    }

    /// Returns the expected hidden items from defaults.
    private func expectedHiddenItemsFromDefaults() -> Set<MenuBarItemInfo> {
        guard
            let data = IceDefaults.data(forKey: .expectedHiddenItems),
    ...
    func hasExpectedHiddenMismatch() -> Bool {
        ...
        return currentHiddenInfos != expectedHidden
    }

    /// Returns the number of expected-hidden items currently missing from hidden.
    func expectedHiddenMissingCount() -> Int {
        let expectedHidden = expectedHiddenItemsFromDefaults()
        guard !expectedHidden.isEmpty else {
            return 0
        }
        ...
        let currentHiddenInfos = Set(itemCache.managedItems(for: .hidden).map(\.info))
        return expectedHidden.subtracting(currentHiddenInfos).count
    }
    ...
    func recoverExpectedHiddenItemsToPolicy(limit: Int? = nil) async -> Bool {
        ...
        guard !isMouseButtonDown, !mouseHasRecentlyMoved, !isMovingItem else {
            return false
        }
        ...
        guard let hiddenControl = items.first(where: { $0.info == .hiddenControlItem }) else {
            return false
        }
        ...
        let limitedMissingCandidates: [MenuBarItem] = if let limit {
            Array(missingCandidates.prefix(max(0, limit)))
        } else {
            missingCandidates
        }

        guard !limitedMissingCandidates.isEmpty || !overflowHiddenItems.isEmpty else {
            return false
        }

        isRecoveringExpectedHiddenItems = true
        MouseCursor.lockUserCursorControl()
        MouseCursor.hide()
        defer {
            MouseCursor.show()
            MouseCursor.unlockUserCursorControl()
            isRecoveringExpectedHiddenItems = false
        }

        var movedAny = false

        if let alwaysHiddenControl {
            for overflowItem in overflowHiddenItems {
                do {
                    try await move(item: overflowItem, to: .leftOfItem(alwaysHiddenControl))
                    movedAny = true
                } catch {
                    Bridging.Logger.itemManager.warning("Failed to route overflow hidden item \(overflowItem.logString): \(error)")
                }
            }
        }

        for candidate in limitedMissingCandidates {
            do {
                try await move(item: candidate, to: .leftOfItem(hiddenControl))
                movedAny = true
            } catch {
                Bridging.Logger.itemManager.warning("Expected-hidden recovery failed for \(candidate.logString): \(error)")
            }
        }

        if movedAny {
            await forceRefreshCache(clearExisting: false)
        }
        return movedAny
    }
    ...
    private func uncheckedCacheItems(
        ...
        Bridging.Logger.itemManager.debug("Caching menu bar items")
    ...
    func cacheItemsIfNeeded() async {
        ...
        guard let hiddenControlItem else {
            Bridging.Logger.itemManager.warning("Missing control item for hidden section")
            Bridging.Logger.itemManager.debug("Keeping existing menu bar item cache")
            cachedItemWindowIDs.removeAll()
            return
        }
        ...
        do {
            ...
            Bridging.Logger.itemManager.error("Error enforcing control item order: \(error)")
            Bridging.Logger.itemManager.debug("Keeping existing menu bar item cache")
            cachedItemWindowIDs.removeAll()
        }
    }
...
extension MenuBarItemManager {
    ...
    private func getCurrentFrame(for item: MenuBarItem) -> CGRect? {
        guard let frame = Bridging.getWindowFrame(for: item.window.windowID) else {
            Bridging.Logger.itemManager.error("Couldn't get current frame for \(item.logString)")
            return nil
        }
        return frame
    }
    ...
    private nonisolated func postEvent(_ event: CGEvent, to location: EventTap.Location) {
        Bridging.Logger.itemManager.debug("Posting \(event.type.logString) to \(location.logString)")
        switch location {
        case .hidEventTap:
    ...
    private func postEventAndWaitToReceive(
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ...
            let eventTap = EventTap(
                ...
                guard let self else {
                    proxy.disable()
                    finish(.failure(EventError(code: .couldNotComplete, item: item)))
                    return nil
                }
                ...
                guard proxy.isEnabled else {
                    Bridging.Logger.itemManager.debug("Event tap \"\(proxy.label)\" is disabled (item: \(item.logString))")
                    return nil
                }

                Bridging.Logger.itemManager.debug("Received \(type.logString) at \(location.logString) (item: \(item.logString))")
            ...
            eventTap.enable(timeout: .milliseconds(50)) {
                Bridging.Logger.itemManager.error("Event tap \"\(eventTap.label)\" timed out (item: \(item.logString))")
                eventTap.disable()
                finish(.failure(EventError(code: .eventOperationTimeout, item: item)))
            }

            watchdogTask = Task.detached {
                try? await Task.sleep(for: .milliseconds(400))
                await eventTap.disable()
                finish(.failure(EventError(code: .eventOperationTimeout, item: item)))
            }
    ...
    private func scrombleEvent(
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ...
            eventTap2.enable(timeout: .milliseconds(50)) {
                Bridging.Logger.itemManager.error("Event tap \"\(eventTap2.label)\" timed out (item: \(item.logString))")
                eventTap1.disable()
                eventTap2.disable()
                finish(.failure(EventError(code: .eventOperationTimeout, item: item)))
            }
            ...
            watchdogTask = Task.detached {
                ...
                await eventTap1.disable()
                await eventTap2.disable()
                finish(.failure(EventError(code: .eventOperationTimeout, item: item)))
            }
    ...
    private func scrombleEvent(
        ...
        guard let currentFrame = getCurrentFrame(for: item) else {
            ...
            Bridging.Logger.itemManager.warning("Couldn't get menu bar item frame for \(item.logString), so using fixed delay")
    ...
    private func waitForFrameChange(of item: MenuBarItem, initialFrame: CGRect, timeout: Duration) async throws {
        ...
        let frameCheckTask = Task(timeout: timeout) {
            while true {
                ...
                if currentFrame != initialFrame {
                    Bridging.Logger.itemManager.debug("Menu bar item frame for \(item.logString) has changed to \(NSStringFromRect(currentFrame))")
                    return
                }
            }
        }
        do {
            ...
            Bridging.Logger.itemManager.warning("Menu bar item frame check for \(item.logString) was cancelled, so using fixed delay")
            // This will be slow, but subsequent events will have a better chance of succeeding.
            try await Task.sleep(for: .milliseconds(50))
        } catch is TaskTimeoutError {
    ...
    private func wakeUpItem(_ item: MenuBarItem) async throws {
        Bridging.Logger.itemManager.debug("Attempting to wake up \(item.logString)")
    ...
    private func moveItemWithoutRestoringMouseLocation(_ item: MenuBarItem, to destination: MoveDestination) async throws {
        ...
        do {
            ...
            do {
                Bridging.Logger.itemManager.debug("Posting fallback event for moving \(item.logString)")
                ...
                Bridging.Logger.itemManager.error("Failed to post fallback event for moving \(item.logString)")
            }
            throw error
        }
    }
    ...
    func move(item: MenuBarItem, to destination: MoveDestination) async throws {
        if try itemHasCorrectPosition(item: item, for: destination) {
            Bridging.Logger.itemManager.debug("\(item.logString) is already in the correct position")
            return
        }
        ...
        Bridging.Logger.itemManager.info("Moving \(item.logString) to \(destination.logString)")
        ...
        for n in 1...5 {
            do {
                ...
                if newFrame != initialFrame {
                    Bridging.Logger.itemManager.info("Successfully moved \(item.logString)")
                    break
                } else {
                    throw EventError(code: .couldNotComplete, item: item)
                }
            } catch where n < 5 {
                Bridging.Logger.itemManager.warning("Attempt \(n) to move \(item.logString) failed (error: \(error))")
                try await wakeUpItem(item)
                Bridging.Logger.itemManager.info("Retrying move of \(item.logString)")
                continue
            }
        }
    }
...
extension MenuBarItemManager {
    ...
    func click(item: MenuBarItem, with mouseButton: CGMouseButton) async throws {
        ...
        do {
            Bridging.Logger.itemManager.info("Clicking \(item.logString) with \(mouseButton.logString)")
            try await postEventAndWaitToReceive(
                mouseDownEvent,
                to: .sessionEventTap,
                item: item
...
extension MenuBarItemManager {
    ...
    private func runTempShownItemTimer(for interval: TimeInterval) {
        Bridging.Logger.itemManager.debug("Automatic temp rehide disabled; not scheduling timer (requested: \(interval))")
    ...
    func tempShowItem(_ item: MenuBarItem, clickWhenFinished: Bool, mouseButton: CGMouseButton) {
        func clickResolvedItemDirectlyIfPossible(_ sourceItem: MenuBarItem, reason: String) {
            guard clickWhenFinished else { return }
            Task {
                let directItem = MenuBarItem(windowID: sourceItem.windowID) ?? sourceItem
                Bridging.Logger.itemManager.info("tempShowItem fallback direct click reason=\(reason) item=\(directItem.logString) windowID=\(directItem.windowID)")
                do {
                    try await click(item: directItem, with: mouseButton)
                } catch {
                    Bridging.Logger.itemManager.error("tempShowItem fallback direct click failed reason=\(reason) error=\(error)")
                }
            }
        }

        func matches(_ lhs: MenuBarItem, _ rhs: MenuBarItem) -> Bool {
            lhs.windowID == rhs.windowID ||
            lhs.info == rhs.info ||
            (lhs.ownerPID == rhs.ownerPID && lhs.info.namespace == rhs.info.namespace)
        }
            ...
            if clickWhenFinished {
                Task {
                    do {
                        ...
                        Bridging.Logger.itemManager.error("ERROR: \(error)")
                    }
                }
            }
            return
        ...
        guard let appState else {
            Bridging.Logger.itemManager.warning("Missing app state, so not showing \(item.logString)")
            clickResolvedItemDirectlyIfPossible(item, reason: "missing_app_state")
            return
        }

        Bridging.Logger.itemManager.info("Temporarily showing \(item.logString)")

        var items = MenuBarItem.getMenuBarItems(onScreenOnly: false, activeSpaceOnly: true)
        var itemToShow = items.first(where: { matches($0, item) })
            ?? items.first(where: { $0.ownerPID == item.ownerPID && $0.title == item.title })
            ?? item
        Bridging.Logger.itemManager.info("tempShowItem resolve input=\(item.logString) windowID=\(item.windowID) resolved=\(itemToShow.logString) resolvedWindowID=\(itemToShow.windowID)")

        var destination = getReturnDestination(for: itemToShow, in: items)
        if destination == nil {
            // Refresh once before giving up in case the hidden-item cache is stale.
            ...
            if let refreshed = items.first(where: { matches($0, item) })
                ?? items.first(where: { $0.ownerPID == item.ownerPID && $0.title == item.title }) {
                itemToShow = refreshed
                destination = getReturnDestination(for: itemToShow, in: items)
            }
        }
        Bridging.Logger.itemManager.info("tempShowItem destination=\(String(describing: destination?.logString)) itemsCount=\(items.count)")

        guard let destination else {
            Bridging.Logger.itemManager.warning("No return destination for \(itemToShow.logString)")
            clickResolvedItemDirectlyIfPossible(itemToShow, reason: "missing_return_destination")
            return
        }

        // Keep temporary movement inside the hidden section by moving directly
        // to the right edge of hidden items (immediately left of the hidden divider).
        guard let targetItem = items.first(where: { $0.info == .hiddenControlItem }) else {
            Bridging.Logger.itemManager.warning("No hidden control item while temp-showing \(itemToShow.logString)")
            clickResolvedItemDirectlyIfPossible(itemToShow, reason: "missing_hidden_control_item")
            return
        }
        Bridging.Logger.itemManager.info("tempShowItem target=\(targetItem.logString) targetWindowID=\(targetItem.windowID)")
        ...
        Task {
            ...
            if clickWhenFinished {
                do {
                    try await slowMove(item: itemToShow, to: .leftOfItem(targetItem))
                    try await click(item: itemToShow, with: mouseButton)
                } catch {
                    Bridging.Logger.itemManager.error("ERROR: \(error)")
                    clickResolvedItemDirectlyIfPossible(itemToShow, reason: "move_or_click_failed")
                }
            } else {
                do {
                    ...
                    Bridging.Logger.itemManager.error("ERROR: \(error)")
                    clickResolvedItemDirectlyIfPossible(itemToShow, reason: "move_failed")
                }
            }
            ...
            let shownInterfaceWindow = currentWindows.first { currentWindow in
                currentWindow.ownerPID == itemToShow.ownerPID &&
            ...
            tempShownItemContexts.removeAll {
                ...
                ($0.info == context.info && $0.ownerPID == context.ownerPID && $0.title == context.title) ||
                ($0.ownerPID == context.ownerPID && $0.info.namespace == context.info.namespace)
            }
    ...
    func rehideTempShownItems() async {
        func matches(_ lhs: MenuBarItem, _ rhs: MenuBarItem) -> Bool {
            lhs.windowID == rhs.windowID ||
            lhs.info == rhs.info ||
            (lhs.ownerPID == rhs.ownerPID && lhs.title == rhs.title) ||
            (lhs.ownerPID == rhs.ownerPID && lhs.info.namespace == rhs.info.namespace)
        }

        func resolvedDestination(
            for context: TempShownItemContext,
            in items: [MenuBarItem]
        ) -> MoveDestination? {
            func fallbackHiddenDestination() -> MoveDestination? {
                guard let hiddenControl = items.first(where: { $0.info == .hiddenControlItem }) else {
                    return nil
                }
            ...
            case .leftOfItem(let target):
                if let latestTarget = items.first(where: { matches($0, target) }) {
                    return .leftOfItem(latestTarget)
                }
                return fallbackHiddenDestination()
            case .rightOfItem(let target):
                if let latestTarget = items.first(where: { matches($0, target) }) {
                    return .rightOfItem(latestTarget)
                }
                return fallbackHiddenDestination()
            }
        }
        ...
        if
            ...
            !hiddenSection.isHidden
        {
            Bridging.Logger.itemManager.info("Hidden section is visible during rehide; forcing hidden state")
            hiddenSection.hide()
            try? await Task.sleep(for: .milliseconds(80))
        }

        guard !isMouseButtonDown else {
            Bridging.Logger.itemManager.debug("Mouse button is down, so waiting to rehide")
            runTempShownItemTimer(for: 3)
            return
        }
        guard !tempShownItemContexts.contains(where: { $0.isShowingInterface && !$0.shouldForceRehide }) else {
            Bridging.Logger.itemManager.debug("Menu bar item interface is shown, so waiting to rehide")
            runTempShownItemTimer(for: 3)
            return
        }

        Bridging.Logger.itemManager.info("Rehiding temporarily shown items")

        var failedContexts = [TempShownItemContext]()

        while let context = tempShownItemContexts.popLast() {
            let items = MenuBarItem.getMenuBarItems(onScreenOnly: false, activeSpaceOnly: false)
            guard let item = items.first(where: { $0.windowID == context.windowID })
                ?? items.first(where: { $0.ownerPID == context.ownerPID && $0.title == context.title })
                ?? items.first(where: { $0.ownerPID == context.ownerPID && $0.info.namespace == context.info.namespace })
                ?? items.first(where: {
                    $0.info.namespace == context.info.namespace &&
                    $0.canBeHidden &&
                    $0.info != .hiddenControlItem &&
                    $0.info != .alwaysHiddenControlItem &&
                    $0.info != .iceIcon
                })
                ?? items.first(where: { $0.info == context.info }) else {
                Bridging.Logger.itemManager.warning("Could not find temporarily shown item during rehide (info: \(context.info))")
                failedContexts.append(context)
                continue
            }
            guard let destination = resolvedDestination(for: context, in: items) else {
                Bridging.Logger.itemManager.warning("Could not resolve destination while rehiding \(item.logString)")
                failedContexts.append(context)
                continue
            }
            do {
                ...
                Bridging.Logger.itemManager.error("Failed to rehide \(item.logString) (error: \(error))")
                failedContexts.append(context)
            }
        }
        ...
        if failedContexts.isEmpty {
            ...
            Bridging.Logger.itemManager.warning("Some items failed to rehide")
    ...
    /// Attempts to recover temporarily shown items by moving them back to the hidden section.
    func recoverTempShownItemsToHiddenSection() async {
        guard !tempShownItemContexts.isEmpty else {
            await forceRefreshCache(clearExisting: false)
            return
        }
        ...
        guard let hiddenControlItem = items.first(where: { $0.info == .hiddenControlItem }) else {
            Bridging.Logger.itemManager.warning("Could not recover hidden items: missing hidden control item")
            cachedItemWindowIDs.removeAll()
            return
        }

        var unresolved = [TempShownItemContext]()
        for context in tempShownItemContexts {
            let candidate = items.first(where: { $0.windowID == context.windowID })
                ?? items.first(where: { $0.ownerPID == context.ownerPID && $0.title == context.title })
                ?? items.first(where: { $0.ownerPID == context.ownerPID && $0.info.namespace == context.info.namespace })
                ?? items.first(where: {
                    $0.info.namespace == context.info.namespace &&
            ...
            guard let candidate else {
                Bridging.Logger.itemManager.warning("Recovery unresolved for temp item \(context.info)")
                unresolved.append(context)
                continue
            }

            do {
                try await move(item: candidate, to: .leftOfItem(hiddenControlItem))
            } catch {
                Bridging.Logger.itemManager.error("Recovery move failed for \(candidate.logString) (error: \(error))")
                unresolved.append(context)
            }
        }

        tempShownItemContexts = unresolved
        await forceRefreshCache(clearExisting: false)
    }
}
...
extension MenuBarItemManager {
    ...
    func enforceControlItemOrder(hiddenControlItem: MenuBarItem, alwaysHiddenControlItem: MenuBarItem) async throws {
        guard !isMouseButtonDown else {
            Bridging.Logger.itemManager.debug("Mouse button is down, so will not enforce control item order")
            return
        }
        guard !mouseHasRecentlyMoved else {
            Bridging.Logger.itemManager.debug("Mouse has recently moved, so will not enforce control item order")
            return
        }
        if hiddenControlItem.frame.maxX <= alwaysHiddenControlItem.frame.minX {
            Bridging.Logger.itemManager.info("Arranging menu bar items")
...
private extension Bridging.Logger {
    ...
    static let itemManager = Bridging.Logger(category: "MenuBarItemManager")
}
